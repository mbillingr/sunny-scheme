...
(define (make-let* bindings body)
  (define (print-bindings bindings)
    (if (null? bindings)
        '()
        (cons (list (caar bindings) ((cdar bindings) 'print))
              (print-bindings (cdr bindings)))))
  (define (print)
    (cons 'LET*
  ...
  (define (transform-bindings bindings fnc)
    (if (null? bindings)
        ...
        (cons (cons (caar bindings)
                    ((cdar bindings) 'transform fnc))
              (transform-bindings (cdr bindings) fnc))))
  (define (transform fnc)
    (fnc self
         (lambda () (make-let*
  ...
  (define (gen-bindings bindings)
    (if (pair? bindings)
        (begin (display "let ")
               (display (caar bindings))
               (display " = ")
               ((cdar bindings) 'gen-rust)
               (display ";")
               (newline)
               (gen-bindings (cdr bindings)))))
  (define (gen-rust)
    (display "{")
