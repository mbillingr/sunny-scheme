...
(define (make-program globals imports body)
  ...
  (define (gen-rust)
    ...
    (rust-block (lambda ()
                  (newline)
                  (display "eprintln!(\"built with\");")
                  (display "eprintln!(\"    '{}' memory model\", MEMORY_MODEL_KIND);")
                  (newline)
                  (body 'gen-rust)
                  (display ";")
                  (newline)))
...
(define (make-import lib)
  ...
  (define (gen-libname lib)
    (if (null? lib)
        (display "")
        (begin (display (rustify-libname (car lib)))
               (if (null? (cdr lib))
                   (display "")
                   (display "::"))
               (gen-libname (cdr lib)))))
...
(define (make-import-only lib names)
  ...
  (define (gen-imports names)
    (if (null? names)
        'DONE
        (begin (display (rustify-identifier (car names)))
               (display ", ")
