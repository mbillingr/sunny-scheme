...
imports::car.with(|value| value.get()).invoke(&[exp_star_.clone(), ]), ]), 
// (list (quote ()))
globals::list.with(|value| value.get()).invoke(&[Scm::Nil, ]), ])} else {
// (program->ast exp*)
...
globals::map.with(|value| value.get()).invoke(&[{Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let b = args[0].clone();
// (letrec () (car b))
{
// (car b)
...
globals::map.with(|value| value.get()).invoke(&[{let body_minus_env = body_minus_env.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let b = args[0].clone();
...
// (define (sexpr->abstraction param* body env) (let ((local-env (adjoin-local-env param* env)) (body (scan-out-defines body))) (if (dotted-list? param*) (make-vararg-abstraction (proper-list-part param*) (last-cdr param*) (map (lambda (p) (lookup p local-env)) param*) (lookup (last-cdr param*) local-env) (sexpr->sequence body local-env #t)) (make-abstraction param* (map (lambda (p) (lookup p local-env)) param*) (sexpr->sequence body local-env #t)))))
...
// (dotted-list? param*)
globals::dotted_minus_list_p.with(|value| value.get()).invoke(&[param_star_.clone(), ])).is_true() {
// (make-vararg-abstraction (proper-list-part param*) (last-cdr param*) (map (lambda (p) (lookup p local-env)) param*) (lookup (last-cdr param*) local-env) (sexpr->sequence body local-env #t))
globals::make_minus_vararg_minus_abstraction.with(|value| value.get()).invoke(&[
// (proper-list-part param*)
globals::proper_minus_list_minus_part.with(|value| value.get()).invoke(&[param_star_.clone(), ]), 
// (last-cdr param*)
globals::last_minus_cdr.with(|value| value.get()).invoke(&[param_star_.clone(), ]), 
// (map (lambda (p) (lookup p local-env)) param*)
globals::map.with(|value| value.get()).invoke(&[{let local_minus_env = local_minus_env.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let p = args[0].clone();
...
// (last-cdr param*)
globals::last_minus_cdr.with(|value| value.get()).invoke(&[param_star_.clone(), ]), local_minus_env.clone(), ]), 
...
// (define (get-lib lib) (let ((full-path (find-library (quote ("." "scm-libs" "../scm-libs")) (library-path lib) (quote (".sld" ".slx"))))) (if full-path (read (open-input-file full-path)) (error "Unknown library" lib))))
globals::get_minus_lib.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let lib = args[0].clone();
// (letrec () (let ((full-path (find-library (quote ("." "scm-libs" "../scm-libs")) (library-path lib) (quote (".sld" ".slx"))))) (if full-path (read (open-input-file full-path)) (error "Unknown library" lib))))
{
// (let ((full-path (find-library (quote ("." "scm-libs" "../scm-libs")) (library-path lib) (quote (".sld" ".slx"))))) (if full-path (read (open-input-file full-path)) (error "Unknown library" lib)))
{let [full_minus_path, ] = [
// (find-library (quote ("." "scm-libs" "../scm-libs")) (library-path lib) (quote (".sld" ".slx")))
globals::find_minus_library.with(|value| value.get()).invoke(&[Scm::pair(Scm::from("."), Scm::pair(Scm::from("scm-libs"), Scm::pair(Scm::from("../scm-libs"), Scm::Nil))), 
// (library-path lib)
...
globals::map.with(|value| value.get()).invoke(&[imports::symbol_minus__g_string.with(|value| value.get()), lib.clone(), ]), ])}})}));
...
globals::map.with(|value| value.get()).invoke(&[globals::importset_minus_libname.with(|value| value.get()), 
// (cdr exp*)
imports::cdr.with(|value| value.get()).invoke(&[exp_star_.clone(), ]), ])}})}));
...
{
// (display "(/*NOP*/)" port)
imports::display.with(|value| value.get()).invoke(&[Scm::from("(/*NOP*/)"), port.clone(), ])}})});
self_.set({let transform = transform.clone();let free_minus_vars = free_minus_vars.clone();let gen_minus_rust = gen_minus_rust.clone();Scm::func(move |args: &[Scm]|{if args.len() < 1{panic!("not enough args")}let msg = args[0].clone();let args_ = Scm::list(&args[1..]);
...
gen_minus_rust.set({let arg = arg.clone();let next = next.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let port = args[0].clone();
// (letrec () (arg (quote gen-rust) port) (display ", " port) (next (quote gen-rust) port))
{{
// (arg (quote gen-rust) port)
arg.clone().invoke(&[Scm::symbol("gen-rust"), port.clone(), ]);
// (display ", " port)
imports::display.with(|value| value.get()).invoke(&[Scm::from(", "), port.clone(), ]);
...
globals::map.with(|value| value.get()).invoke(&[{let fnc = fnc.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let a = args[0].clone();
...
gen_minus_rust.set({let params = params.clone();let args_ = args_.clone();let body = body.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let port = args[0].clone();
// (letrec () (rust-block port (lambda () (for-each (lambda (p) (println port "let " (rustify-identifier p) " = Scm::uninitialized().into_boxed();")) params) (for-each2 (lambda (p a) (print port (rustify-identifier p) ".set(") (a (quote gen-rust) port) (println port ");")) params args) (body (quote gen-rust) port))))
{
// (rust-block port (lambda () (for-each (lambda (p) (println port "let " (rustify-identifier p) " = Scm::uninitialized().into_boxed();")) params) (for-each2 (lambda (p a) (print port (rustify-identifier p) ".set(") (a (quote gen-rust) port) (println port ");")) params args) (body (quote gen-rust) port)))
...
// (letrec () (for-each (lambda (p) (println port "let " (rustify-identifier p) " = Scm::uninitialized().into_boxed();")) params) (for-each2 (lambda (p a) (print port (rustify-identifier p) ".set(") (a (quote gen-rust) port) (println port ");")) params args) (body (quote gen-rust) port))
{{
// (for-each (lambda (p) (println port "let " (rustify-identifier p) " = Scm::uninitialized().into_boxed();")) params)
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let p = args[0].clone();
...
// (for-each2 (lambda (p a) (print port (rustify-identifier p) ".set(") (a (quote gen-rust) port) (println port ");")) params args)
globals::for_minus_each2.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let p = args[0].clone();let a = args[1].clone();
...
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let i = args[0].clone();
...
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let lib = args[0].clone();
...
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let l = args[0].clone();
...
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let expo = args[0].clone();
...
globals::for_minus_each.with(|value| value.get()).invoke(&[{let module_minus_tree = module_minus_tree.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let lib = args[0].clone();
...
globals::rust_minus_gen_minus_module_minus_tree.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let port = args[0].clone();let node = args[1].clone();
...
globals::rust_minus_gen_minus_module_minus_tree_minus_list.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let port = args[0].clone();let nodes = args[1].clone();
...
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let child = args[0].clone();
// (letrec () (rust-gen-module-tree port child))
{
// (rust-gen-module-tree port child)
globals::rust_minus_gen_minus_module_minus_tree.with(|value| value.get()).invoke(&[port.clone(), child.clone(), ])}})}, nodes.clone(), ])}})}));
...
globals::assq.with(|value| value.get()).invoke(&[name.clone(), 
// (module-tree-children node)
globals::module_minus_tree_minus_children.with(|value| value.get()).invoke(&[node.clone(), ]), ])}}})}));
...
globals::rust_minus_block.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let port = args[0].clone();let code = args[1].clone();
...
globals::map.with(|value| value.get()).invoke(&[char_minus_map.get(), 
...
globals::list.with(|value| value.get()).invoke(&[Scm::symbol("GLOBAL-MARKER"), 
...
globals::list.with(|value| value.get()).invoke(&[getter.clone(), setter.clone(), mut_p.clone(), ])}})}));
...
globals::dotted_minus_list_p.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let seq = args[0].clone();
// (letrec () (cond ((null? seq) #f) ((pair? seq) (dotted-list? (cdr seq))) (else #t)))
{
// (cond ((null? seq) #f) ((pair? seq) (dotted-list? (cdr seq))) (else #t))
if (
// (null? seq)
imports::null_p.with(|value| value.get()).invoke(&[seq.clone(), ])).is_true() {Scm::False} else {if (
// (pair? seq)
imports::pair_p.with(|value| value.get()).invoke(&[seq.clone(), ])).is_true() {
// (dotted-list? (cdr seq))
globals::dotted_minus_list_p.with(|value| value.get()).invoke(&[
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ])} else {Scm::True}}}})}));
// (define (last-cdr seq) (if (pair? seq) (last-cdr (cdr seq)) seq))
globals::last_minus_cdr.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let seq = args[0].clone();
...
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ])} else {seq.clone()}}})}));
// (define (proper-list-part seq) (if (pair? seq) (cons (car seq) (proper-list-part (cdr seq))) (quote ())))
globals::proper_minus_list_minus_part.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let seq = args[0].clone();
...
globals::proper_minus_list_minus_part.with(|value| value.get()).invoke(&[
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ]), ])} else {Scm::Nil}}})}));
// (define (println port . args) (for-each (lambda (a) (display a port)) args) (newline port))
globals::println.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() < 1{panic!("not enough args")}let port = args[0].clone();let args_ = Scm::list(&args[1..]);
// (letrec () (for-each (lambda (a) (display a port)) args) (newline port))
{{
// (for-each (lambda (a) (display a port)) args)
globals::for_minus_each.with(|value| value.get()).invoke(&[{let port = port.clone();Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let a = args[0].clone();
// (letrec () (display a port))
{
// (display a port)
imports::display.with(|value| value.get()).invoke(&[a.clone(), port.clone(), ])}})}, args_.clone(), ]);
// (newline port)
imports::newline.with(|value| value.get()).invoke(&[port.clone(), ])}}})}));
// (define (print port . args) (for-each (lambda (a) (display a port)) args))
globals::print.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() < 1{panic!("not enough args")}let port = args[0].clone();let args_ = Scm::list(&args[1..]);
...
globals::writeln.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() < 1{panic!("not enough args")}let port = args[0].clone();let args_ = Scm::list(&args[1..]);
...
{
// (write a port)
imports::write.with(|value| value.get()).invoke(&[a.clone(), port.clone(), ])}})}, args_.clone(), ]);
// (newline port)
imports::newline.with(|value| value.get()).invoke(&[port.clone(), ])}}})}));
// (define (append! seq-a seq-b) (if (null? (cdr seq-a)) (set-cdr! seq-a seq-b) (append! (cdr seq-a) seq-b)))
globals::append_i.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let seq_minus_a = args[0].clone();let seq_minus_b = args[1].clone();
// (letrec () (if (null? (cdr seq-a)) (set-cdr! seq-a seq-b) (append! (cdr seq-a) seq-b)))
{if (
// (null? (cdr seq-a))
imports::null_p.with(|value| value.get()).invoke(&[
// (cdr seq-a)
imports::cdr.with(|value| value.get()).invoke(&[seq_minus_a.clone(), ]), ])).is_true() {
// (set-cdr! seq-a seq-b)
imports::set_minus_cdr_i.with(|value| value.get()).invoke(&[seq_minus_a.clone(), seq_minus_b.clone(), ])} else {
// (append! (cdr seq-a) seq-b)
globals::append_i.with(|value| value.get()).invoke(&[
// (cdr seq-a)
imports::cdr.with(|value| value.get()).invoke(&[seq_minus_a.clone(), ]), seq_minus_b.clone(), ])}}})}));
// (define (not x) (if x #f #t))
globals::not.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let x = args[0].clone();
// (letrec () (if x #f #t))
{if (x.clone()).is_true() {Scm::False} else {Scm::True}}})}));
// (define (list . x) x)
globals::list.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() < 0{panic!("not enough args")}let x = Scm::list(&args[0..]);
// (letrec () x)
{x.clone()}})}));
// (define (length seq) (if (pair? seq) (+ 1 (length (cdr seq))) 0))
globals::length.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let seq = args[0].clone();
...
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ]), ])} else {Scm::from(0)}}})}));
// (define (for-each f seq) (if (pair? seq) (begin (f (car seq)) (for-each f (cdr seq)))))
globals::for_minus_each.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let f = args[0].clone();let seq = args[1].clone();
// (letrec () (if (pair? seq) (begin (f (car seq)) (for-each f (cdr seq)))))
{if (
// (pair? seq)
imports::pair_p.with(|value| value.get()).invoke(&[seq.clone(), ])).is_true() {{
// (f (car seq))
...
imports::car.with(|value| value.get()).invoke(&[seq.clone(), ]), ]);
// (for-each f (cdr seq))
globals::for_minus_each.with(|value| value.get()).invoke(&[f.clone(), 
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ])}} else {Scm::symbol("*UNSPECIFIED*")}}})}));
// (define (for-each2 f seq-a seq-b) (if (and (pair? seq-a) (pair? seq-b)) (begin (f (car seq-a) (car seq-b)) (for-each2 f (cdr seq-a) (cdr seq-b)))))
globals::for_minus_each2.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 3{panic!("invalid arity")}let f = args[0].clone();let seq_minus_a = args[1].clone();let seq_minus_b = args[2].clone();
// (letrec () (if (and (pair? seq-a) (pair? seq-b)) (begin (f (car seq-a) (car seq-b)) (for-each2 f (cdr seq-a) (cdr seq-b)))))
{if (
// (and (pair? seq-a) (pair? seq-b))
if (
// (pair? seq-a)
imports::pair_p.with(|value| value.get()).invoke(&[seq_minus_a.clone(), ])).is_true() {
// (pair? seq-b)
imports::pair_p.with(|value| value.get()).invoke(&[seq_minus_b.clone(), ])} else {Scm::False}).is_true() {{
// (f (car seq-a) (car seq-b))
f.clone().invoke(&[
// (car seq-a)
imports::car.with(|value| value.get()).invoke(&[seq_minus_a.clone(), ]), 
// (car seq-b)
imports::car.with(|value| value.get()).invoke(&[seq_minus_b.clone(), ]), ]);
// (for-each2 f (cdr seq-a) (cdr seq-b))
globals::for_minus_each2.with(|value| value.get()).invoke(&[f.clone(), 
// (cdr seq-a)
imports::cdr.with(|value| value.get()).invoke(&[seq_minus_a.clone(), ]), 
// (cdr seq-b)
imports::cdr.with(|value| value.get()).invoke(&[seq_minus_b.clone(), ]), ])}} else {Scm::symbol("*UNSPECIFIED*")}}})}));
// (define (map f seq) (if (pair? seq) (cons (f (car seq)) (map f (cdr seq))) (quote ())))
globals::map.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let f = args[0].clone();let seq = args[1].clone();
// (letrec () (if (pair? seq) (cons (f (car seq)) (map f (cdr seq))) (quote ())))
{if (
// (pair? seq)
imports::pair_p.with(|value| value.get()).invoke(&[seq.clone(), ])).is_true() {
...
imports::car.with(|value| value.get()).invoke(&[seq.clone(), ]), ]), 
// (map f (cdr seq))
globals::map.with(|value| value.get()).invoke(&[f.clone(), 
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ]), ])} else {Scm::Nil}}})}));
...
globals::memq.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let obj = args[0].clone();let seq = args[1].clone();
// (letrec () (if (pair? seq) (if (eq? obj (car seq)) seq (memq obj (cdr seq))) #f))
{if (
// (pair? seq)
imports::pair_p.with(|value| value.get()).invoke(&[seq.clone(), ])).is_true() {if (
// (eq? obj (car seq))
imports::eq_p.with(|value| value.get()).invoke(&[obj.clone(), 
// (car seq)
imports::car.with(|value| value.get()).invoke(&[seq.clone(), ]), ])).is_true() {seq.clone()} else {
// (memq obj (cdr seq))
globals::memq.with(|value| value.get()).invoke(&[obj.clone(), 
// (cdr seq)
imports::cdr.with(|value| value.get()).invoke(&[seq.clone(), ]), ])}} else {Scm::False}}})}));
...
globals::assq.with(|value| value.set({Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let obj = args[0].clone();let seq = args[1].clone();
...
globals::output_minus_file_minus_name.with(|value| value.set(
// (caddr (command-line))
...
imports::display.with(|value| value.get()).invoke(&[globals::output_minus_file_minus_name.with(|value| value.get()), ]);
// (newline)
imports::newline.with(|value| value.get()).invoke(&[]);
// (newline)
imports::newline.with(|value| value.get()).invoke(&[]);
...
// (scm->ast program)
globals::scm_minus__g_ast.with(|value| value.get()).invoke(&[globals::program.with(|value| value.get()), ])));
// (define output-file (open-output-file output-file-name))
globals::output_minus_file.with(|value| value.set(
// (open-output-file output-file-name)
imports::open_minus_output_minus_file.with(|value| value.get()).invoke(&[globals::output_minus_file_minus_name.with(|value| value.get()), ])));
// (ast (quote gen-rust) output-file)
globals::ast.with(|value| value.get()).invoke(&[Scm::symbol("gen-rust"), globals::output_minus_file.with(|value| value.get()), ]);
// (close-port output-file)
imports::close_minus_port.with(|value| value.get()).invoke(&[globals::output_minus_file.with(|value| value.get()), ])};
}
pub mod scheme {
pub mod base {
#[allow(unused_imports)] use sunny_core::{Mut, Scm};
mod imports{pub use crate::native::base::exports::*;
}

pub mod exports{pub use super::imports::_e_ as _e_;
pub use super::imports::_g_ as _g_;
pub use super::imports::_l_ as _l_;
pub use super::imports::_minus_ as _minus_;
pub use super::imports::_plus_ as _plus_;
pub use super::imports::car as car;
pub use super::imports::caar as caar;
pub use super::imports::cadr as cadr;
pub use super::imports::cdr as cdr;
pub use super::imports::cdar as cdar;
pub use super::imports::cddr as cddr;
pub use super::imports::char_p as char_p;
pub use super::imports::close_minus_port as close_minus_port;
pub use super::imports::cons as cons;
pub use super::imports::eof_minus_object_p as eof_minus_object_p;
pub use super::imports::eq_p as eq_p;
pub use super::imports::equal_p as equal_p;
pub use super::imports::error as error;
pub use super::imports::list_minus__g_string as list_minus__g_string;
pub use super::imports::null_p as null_p;
pub use super::imports::pair_p as pair_p;
pub use super::imports::set_minus_car_i as set_minus_car_i;
pub use super::imports::set_minus_cdr_i as set_minus_cdr_i;
pub use super::imports::string_minus__g_list as string_minus__g_list;
pub use super::imports::string_minus_append as string_minus_append;
pub use super::imports::string_l__p as string_l__p;
pub use super::imports::symbol_p as symbol_p;
pub use super::imports::symbol_minus__g_string as symbol_minus__g_string;
}
...
#[allow(unused_imports)] use sunny_core::{Mut, Scm};
mod imports{pub use crate::native::process_context::exports::*;
}

pub mod exports{pub use super::imports::command_minus_line as command_minus_line;
}
