...
pub fn rust_minus_gen_minus_global_minus_defs(args: &[Scm]) -> Scm {
    {if args.len() != 2{panic!("invalid arity")}let module__5 = args[0].clone();let g__0 = args[1].clone();if ({
// (null? g)
imports::null_p(&[g__0.clone()])}).is_true() {{
// (println module)
imports::println(&[module__5.clone()])}} else {{
// (cond ...)
if ({
// (import-variable? (car g))
imports::import_minus_variable_p(&[{
// (car g)
imports::car(&[g__0.clone()])}])}).is_true() {{
// (rust-gen-global-defs module (cdr g))
Scm::func(rust_minus_gen_minus_global_minus_defs).invoke(&[module__5.clone(),{
// (cdr g)
imports::cdr(&[g__0.clone()])}])}} else if ({
// (keyword? (car g))
imports::keyword_p(&[{
// (car g)
imports::car(&[g__0.clone()])}])}).is_true() {{
...
imports::println(&[module__5.clone(),Scm::from("thread_local!{#[allow(non_upper_case_globals)] pub static "),{
// (rustify-identifier (variable-name (car g)))
imports::rustify_minus_identifier(&[{
// (variable-name (car g))
imports::variable_minus_name(&[{
// (car g)
imports::car(&[g__0.clone()])}])}])},Scm::from(": Mut<Scm> = Mut::new(Scm::symbol(\"UNINITIALIZED GLOBAL VARIABLE "),{
// (variable-name (car g))
imports::variable_minus_name(&[{
// (car g)
imports::car(&[g__0.clone()])}])},Scm::from("\"))}")])};{
// (rust-gen-global-defs module (cdr g))
Scm::func(rust_minus_gen_minus_global_minus_defs).invoke(&[module__5.clone(),{
// (cdr g)
imports::cdr(&[g__0.clone()])}])}}} else if ({
...
imports::println(&[module__5.clone(),Scm::from("pub fn "),{
// (rustify-identifier (variable-name (car g)))
imports::rustify_minus_identifier(&[{
// (variable-name (car g))
imports::variable_minus_name(&[{
// (car g)
imports::car(&[g__0.clone()])}])}])},Scm::from("(args: &[Scm]) -> Scm { ")])};{
...
// (car g)
imports::car(&[g__0.clone()])}])}.invoke(&[Scm::symbol("gen-rust"),module__5.clone()])};{
// (println module ".into()}")
imports::println(&[module__5.clone(),Scm::from(".into()}")])};{
// (rust-gen-global-defs module (cdr g))
Scm::func(rust_minus_gen_minus_global_minus_defs).invoke(&[module__5.clone(),{
// (cdr g)
imports::cdr(&[g__0.clone()])}])}}} else {{
// (error "Unexpected entry in global environment" (car g))
imports::error(&[Scm::from("Unexpected entry in global environment"),{
// (car g)
imports::car(&[g__0.clone()])}])}}}}}.into()
}
pub fn rust_minus_gen_minus_in_minus_module(args: &[Scm]) -> Scm {
    {
        ...
        let name__29 = args[0].clone();
        let base_minus_path__1 = args[1].clone();
        let body__0 = args[2].clone();
        {
            // (let ((module (open-module name base-path))) (body module) (close-module module))
            {
                let module__9 = {
                    // (open-module name base-path)
                    imports::open_minus_module(&[name__29.clone(), base_minus_path__1.clone()])
                };
                {
                    {
                        // (body module)
                        body__0.clone().invoke(&[module__9.clone()])
                    };
                    {
                        // (close-module module)
                        imports::close_minus_module(&[module__9.clone()])
                    }
                }
            }
        }
...
pub fn rust_minus_gen_minus_in_minus_submodule(args: &[Scm]) -> Scm {
    {
        ...
        let name__30 = args[0].clone();
        let parent__1 = args[1].clone();
        let body__1 = args[2].clone();
        {
            // (let ((module (open-submodule name parent))) (body module) (close-module module))
            {
                let module__10 = {
                    // (open-submodule name parent)
                    imports::open_minus_submodule(&[name__30.clone(), parent__1.clone()])
                };
                {
                    {
                        // (body module)
                        body__1.clone().invoke(&[module__10.clone()])
                    };
                    {
                        // (close-module module)
                        imports::close_minus_module(&[module__10.clone()])
                    }
                }
            }
        }
...
pub fn rust_minus_gen_minus_module_minus_tree(args: &[Scm]) -> Scm {
    {
        ...
        let module__7 = args[0].clone();
        let node__7 = args[1].clone();
        {
            {
                ...
                imports::println(&[
                    ...
                    Scm::from("pub mod "),
                    {
                        ...
                        imports::rustify_minus_libname(&[{
                            ...
                            imports::module_minus_tree_minus_name(&[node__7.clone()])
                        }])
                    },
                    Scm::from(";"),
                ])
            ...
            if ({
                ...
                imports::module_minus_tree_minus_leaf_p(&[node__7.clone()])
            })
            .is_true()
            {
                {
                    ...
                    Scm::func(rust_minus_gen_minus_in_minus_submodule).invoke(&[
                        ...
                        {
                            // Closure
                            let node__7 = node__7.clone();
                            Scm::func(move |args: &[Scm]| {
                                if args.len() != 1 {
                                    panic!("invalid arity")
                                }
                                let submod__0 = args[0].clone();
                                {
                                    // ((module-tree-libobj node) (quote gen-rust) submod)
                                    {
                                        // (module-tree-libobj node)
                                        imports::module_minus_tree_minus_libobj(&[node__7.clone()])
                                    }
                                    .invoke(&[Scm::symbol("gen-rust"), submod__0.clone()])
                                }
                            })
                        },
                    ])
...
pub fn rust_minus_gen_minus_module_minus_tree_minus_list(args: &[Scm]) -> Scm {
    {
        ...
        let module__8 = args[0].clone();
        let nodes__0 = args[1].clone();
        {
            // (for-each (lambda (child) (rust-gen-module-tree module child)) nodes)
            imports::for_minus_each(&[
                {
                    // Closure
                    let module__8 = module__8.clone();
                    Scm::func(move |args: &[Scm]| {
                        if args.len() != 1 {
                            panic!("invalid arity")
                        }
                        let child__2 = args[0].clone();
                        {
...
pub fn rust_minus_gen_minus_modules(args: &[Scm]) -> Scm {
    {
        ...
        let module__6 = args[0].clone();
        let libs__0 = args[1].clone();
        {
                ...
                {
                    {
                        ...
                        imports::for_minus_each(&[
                            {
                                // Closure
                                let module_minus_tree__0 = module_minus_tree__0.clone();
                                Scm::func(move |args: &[Scm]| {
                                    if args.len() != 1 {
                                        panic!("invalid arity")
                                    }
                                    let lib__1 = args[0].clone();
                                    {
                                        // (module-tree-insert! module-tree (car lib) (cdr lib))
                                        imports::module_minus_tree_minus_insert_i(&[
                                            module_minus_tree__0.clone(),
                                            {
                                                // (car lib)
                                                imports::car(&[lib__1.clone()])
                                            },
                                            {
                                                // (cdr lib)
                                                imports::cdr(&[lib__1.clone()])
                                            },
                                        ])
                                    }
                                })
