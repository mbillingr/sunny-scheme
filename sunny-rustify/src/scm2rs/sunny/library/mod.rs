...
pub fn check_minus_imports(args: &[Scm]) -> Scm {
    {
        ...
        let imports__3 = args[0].clone();
        let exports__3 = args[1].clone();
        let lib__10 = args[2].clone();
        if ({
            // (null? imports)
            imports::null_p(&[imports__3.clone()])
        })
        .is_true()
        {
            Scm::True
...
pub fn find_minus_library(args: &[Scm]) -> Scm {
    {
        ...
        let base_minus_path_star___0 = args[0].clone();
        let relative_minus_path__0 = args[1].clone();
        let extension_star___0 = args[2].clone();
        if ({
            // (null? base-path*)
            imports::null_p(&[base_minus_path_star___0.clone()])
        })
        .is_true()
        {
            Scm::False
                    ...
                    // (let ((path (string-append (car base-path*) relative-path))) (let ((full-path (find-library-ext path extension*))) (begin (if full-path full-path (find-library (cdr base-path*) relative-path extension*)))))
                    {
                        let path__1 = {
                            ...
                            imports::string_minus_append(&[
                                {
                                    // (car base-path*)
                                    imports::car(&[base_minus_path_star___0.clone()])
                                },
                                relative_minus_path__0.clone(),
                            ])
                        };
                        ...
                        let full_minus_path__1 = {
                            // (find-library-ext path extension*)
                            Scm::func(find_minus_library_minus_ext)
                                .invoke(&[path__1.clone(), extension_star___0.clone()])
                        };
                        if (full_minus_path__1.clone()).is_true() {
                            full_minus_path__1.clone()
                        } else {
                            {
                                ...
                                Scm::func(find_minus_library).invoke(&[
                                    {
                                        // (cdr base-path*)
                                        imports::cdr(&[base_minus_path_star___0.clone()])
                                    },
                                    relative_minus_path__0.clone(),
                                    extension_star___0.clone(),
                                ])
                            }
                        }
                    }
...
pub fn find_minus_library_minus_ext(args: &[Scm]) -> Scm {
    {
        ...
        let path__2 = args[0].clone();
        let extension_star___1 = args[1].clone();
        if ({
            // (null? extension*)
            imports::null_p(&[extension_star___1.clone()])
        })
        .is_true()
        {
            Scm::False
        } else {
            {
                ...
                {
                    let full_minus_path__2 = {
                        // (string-append path (car extension*))
                        imports::string_minus_append(&[path__2.clone(), {
                            // (car extension*)
                            imports::car(&[extension_star___1.clone()])
                        }])
                    };
                    if ({
                        // (file-exists? full-path)
                        imports::file_minus_exists_p(&[full_minus_path__2.clone()])
                    })
                    .is_true()
                    {
                        full_minus_path__2.clone()
                    } else {
                        {
                            // (find-library-ext path (cdr extension*))
                            Scm::func(find_minus_library_minus_ext).invoke(&[path__2.clone(), {
                                // (cdr extension*)
                                imports::cdr(&[extension_star___1.clone()])
                            }])
                        }
                    }
                }
            ...
            // (let ((full-path (find-library (quote ("." "./lib" "./scheme/lib" "scm-libs" "../scheme/lib" "../scm-libs" "../../scm-libs")) (library-path lib) (quote (".sld" ".slx"))))) (if full-path (read (open-input-file full-path)) (error "Unknown library" lib)))
            {
                let full_minus_path__0 = {
                    ...
                    Scm::func(find_minus_library).invoke(&[
                        ...
                        {
                            // (library-path lib)
                            Scm::func(library_minus_path).invoke(&[lib__8.clone()])
                        },
                        Scm::pair(Scm::from(".sld"), Scm::pair(Scm::from(".slx"), Scm::Nil)),
                    ])
                };
                if (full_minus_path__0.clone()).is_true() {
                    {
                        // (read (open-input-file full-path))
                        imports::read(&[{
                            // (open-input-file full-path)
                            imports::open_minus_input_minus_file(&[full_minus_path__0.clone()])
                        }])
                    }
                } else {
                    {
                        // (error "Unknown library" lib)
                        imports::error(&[Scm::from("Unknown library"), lib__8.clone()])
                    }
                }
            }
        }
...
pub fn library_minus_decls(args: &[Scm]) -> Scm {
    {
        ...
        let expr__1 = args[0].clone();
        {
            // (cddr expr)
            imports::cddr(&[expr__1.clone()])
        }
    }
    .into()
}
pub fn library_minus_exports(args: &[Scm]) -> Scm {
    {
        ...
        let lib_minus_decl_star___0 = args[0].clone();
        {
            // (cond ...)
            if ({
                // (null? lib-decl*)
                imports::null_p(&[lib_minus_decl_star___0.clone()])
            })
            .is_true()
            {
                Scm::Nil
                ...
                imports::eq_p(&[Scm::symbol("export"), {
                    // (caar lib-decl*)
                    imports::caar(&[lib_minus_decl_star___0.clone()])
                }])
            })
            .is_true()
            {
                {
                    ...
                    imports::append(&[
                        {
                            // (cdar lib-decl*)
                            imports::cdar(&[lib_minus_decl_star___0.clone()])
                        },
                        {
                            // (library-exports (cdr lib-decl*))
                            Scm::func(library_minus_exports).invoke(&[{
                                // (cdr lib-decl*)
                                imports::cdr(&[lib_minus_decl_star___0.clone()])
                            }])
                        },
                    ])
                }
...
pub fn library_minus_name(args: &[Scm]) -> Scm {
    {
        ...
        let expr__0 = args[0].clone();
        {
            // (cadr expr)
            imports::cadr(&[expr__0.clone()])
        }
    }
    .into()
}
            ...
            imports::reduce(&[
                {
                    ...
                    Scm::func(move |args: &[Scm]| {
                        ...
                        let left__0 = args[0].clone();
                        let right__0 = args[1].clone();
                        {
                            // (string-append left (string-append "/" right))
                            imports::string_minus_append(&[left__0.clone(), {
                                // (string-append "/" right)
                                imports::string_minus_append(&[Scm::from("/"), right__0.clone()])
                            }])
                        }
                    })
                },
                Scm::from(""),
                {
                    ...
                    imports::map(&[Scm::func(imports::symbol_minus__g_string), lib__9.clone()])
                },
            ])
        }
    }
