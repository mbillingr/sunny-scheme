...
pub fn initialize() {
    ...
    crate::sunny::utils::initialize();
    {
        ...
        };
        {
            ...
            globals::astify_minus_library.with(|value| value.set({// Closure
Scm::func(move |args: &[Scm]|{if args.len() != 3{panic!("invalid arity")}let name = args[0].clone();let exp_star_ = args[1].clone();let library_minus_env = args[2].clone();{
...
// (make-core-env)
imports::make_minus_core_minus_env.with(|value| value.get()).invoke(&[])});imports.set(Scm::Nil);exports.set(Scm::Nil);process_minus_library_minus_decls.set({// Closure
...
imports::close_minus_procedures.with(|value| value.get()).invoke(&[body.get()])}])},imports.get(),exports.get()])}}}}}}}}}}}}}}})}))
        };
        {
            ...
            globals::astify_minus_program.with(|value| value.set({// Closure
Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let exp_star_ = args[0].clone();{
...
// (list (quote ()))
imports::list.with(|value| value.get()).invoke(&[Scm::Nil])});process_minus_imports.set({// Closure
...
imports::make_minus_set.with(|value| value.get()).invoke(&[])}])}}}}}}}}})}))
        };
        {
            ...
            globals::register_minus_libraries.with(|value| value.set({// Closure
Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let libs = args[0].clone();let library_minus_env = args[1].clone();{
// (cond ...)
if ({
// (null? libs)
