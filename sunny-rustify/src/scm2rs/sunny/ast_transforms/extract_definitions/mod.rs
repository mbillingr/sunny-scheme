...
{let [transform__698, extract_minus_definition__700, ] = [Scm::symbol("*uninitialized*"),Scm::symbol("*uninitialized*")];{let extract_minus_definition__700 = extract_minus_definition__700.into_boxed();{let transform__698 = transform__698.into_boxed();{transform__698.set({// Closure
let extract_minus_definition__700 = extract_minus_definition__700.clone();Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let node__699 = args[0].clone();let transform_minus_children__701 = args[1].clone();{
// (cond ...)
if ({
// (eq? (node (quote kind)) (quote DEFINITION))
imports::eq_p(&[{
// (node (quote kind))
node__699.clone().invoke(&[Scm::symbol("kind")])},Scm::symbol("DEFINITION")])}).is_true() {{
// (extract-definition node)
extract_minus_definition__700.get().invoke(&[node__699.clone()])}} else {{
// (transform-children)
transform_minus_children__701.clone().invoke(&[])}}}})});Scm::anything();extract_minus_definition__700.set({// Closure
Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let node__702 = args[0].clone();{
...
{let val__703 = {
// (node (quote get-val))
node__702.clone().invoke(&[Scm::symbol("get-val")])};{
// (cond ...)
if ({
// (eq? (quote CLOSURE) (val (quote kind)))
imports::eq_p(&[Scm::symbol("CLOSURE"),{
// (val (quote kind))
val__703.clone().invoke(&[Scm::symbol("kind")])}])}).is_true() {{if ({
// (not (null? (val (quote free-vars))))
imports::not(&[{
// (null? (val (quote free-vars)))
imports::null_p(&[{
// (val (quote free-vars))
val__703.clone().invoke(&[Scm::symbol("free-vars")])}])}])}).is_true() {{
...
// (node (quote get-var))
node__702.clone().invoke(&[Scm::symbol("get-var")])}])},{
// (val (quote free-vars))
val__703.clone().invoke(&[Scm::symbol("free-vars")])}])}} else {Scm::symbol("*UNSPECIFIED*")};{
...
val__703.clone().invoke(&[Scm::symbol("inner-function")])}])};{
// (make-nop)
imports::make_minus_nop(&[])}}} else {{
// (make-definition (node (quote get-var)) val)
imports::make_minus_definition(&[{
// (node (quote get-var))
node__702.clone().invoke(&[Scm::symbol("get-var")])},val__703.clone()])}}}}}})});Scm::anything();{
