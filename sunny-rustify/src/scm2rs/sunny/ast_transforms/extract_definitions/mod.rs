...
pub fn extract_minus_definitions(args: &[Scm]) -> Scm {
    {if args.len() != 1{panic!("invalid arity")}let node__16 = args[0].clone();{
...
{let [transform__27, extract_minus_definition__0, ] = [Scm::symbol("*uninitialized*"),Scm::symbol("*uninitialized*")];{let extract_minus_definition__0 = extract_minus_definition__0.into_boxed();{let transform__27 = transform__27.into_boxed();{transform__27.set({// Closure
let extract_minus_definition__0 = extract_minus_definition__0.clone();Scm::func(move |args: &[Scm]|{if args.len() != 2{panic!("invalid arity")}let node__14 = args[0].clone();let transform_minus_children__1 = args[1].clone();{
// (cond ...)
if ({
// (eq? (node (quote kind)) (quote DEFINITION))
imports::eq_p(&[{
// (node (quote kind))
node__14.clone().invoke(&[Scm::symbol("kind")])},Scm::symbol("DEFINITION")])}).is_true() {{
// (extract-definition node)
extract_minus_definition__0.get().invoke(&[node__14.clone()])}} else {{
// (transform-children)
transform_minus_children__1.clone().invoke(&[])}}}})});Scm::anything();extract_minus_definition__0.set({// Closure
Scm::func(move |args: &[Scm]|{if args.len() != 1{panic!("invalid arity")}let node__15 = args[0].clone();{
...
{let val__6 = {
// (node (quote get-val))
node__15.clone().invoke(&[Scm::symbol("get-val")])};{
// (cond ...)
if ({
// (eq? (quote CLOSURE) (val (quote kind)))
imports::eq_p(&[Scm::symbol("CLOSURE"),{
// (val (quote kind))
val__6.clone().invoke(&[Scm::symbol("kind")])}])}).is_true() {{if ({
// (not (null? (val (quote free-vars))))
imports::not(&[{
// (null? (val (quote free-vars)))
imports::null_p(&[{
// (val (quote free-vars))
val__6.clone().invoke(&[Scm::symbol("free-vars")])}])}])}).is_true() {{
...
// (node (quote get-var))
node__15.clone().invoke(&[Scm::symbol("get-var")])}])},{
// (val (quote free-vars))
val__6.clone().invoke(&[Scm::symbol("free-vars")])}])}} else {Scm::symbol("*UNSPECIFIED*")};{
...
val__6.clone().invoke(&[Scm::symbol("inner-function")])}])};{
// (make-nop)
imports::make_minus_nop(&[])}}} else {{
// (make-definition (node (quote get-var)) val)
imports::make_minus_definition(&[{
// (node (quote get-var))
node__15.clone().invoke(&[Scm::symbol("get-var")])},val__6.clone()])}}}}}})});Scm::anything();{
// (node (quote transform) transform)
node__16.clone().invoke(&[Scm::symbol("transform"),transform__27.get()])}}}}}}}}.into()
}
